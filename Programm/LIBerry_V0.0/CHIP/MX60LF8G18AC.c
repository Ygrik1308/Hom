...
#if defined MT29F32G08AFABA    // chip
/* Includes ------------------------------------------------------------------*/
//#include "main.h"
...
#include "main.h"
    ...
    #define WP     GPIOC,5  // OUTPUT  оранджевый

////////////////////////////////////////////////////////
Address_type create_address( uint16_t die,
							 uint16_t plane,
							 uint16_t block,               //2048
							 uint16_t page,                //32
							 uint16_t byte )               //2048 - data + 64 -  ,4095
{

//	  HAL_Delay(100);
  Address_type at = {
		.column_addr1 = byte & 0xff,                                   //++
		.column_addr2 = ( byte >> 8 ) & 0x0f,                          //++
		.row_addr3    =  ( page & 0x3f ) | (( plane & 0x01 ) << 6) |(( block & 0x01 ) << 7 ),    //
		.row_addr4    = ( block >> 1 ) & 0xff,
		.row_addr5    = (( block >> 9 ) & 0x03 ) | (( die & 0x01 ) << 2 )  //9
	};
  return at;
	  	   ...
	  	   for(uint16_t h=0; h<2; ++h )         //h  plane
	 	     {
	  		   for(uint16_t k=0; k<2049; ++k )    //k bloks
	  		   {
	  			   ...
	  			   {
	  	  	              address = create_address(m,h,k,j,2048);  //?2047
	  	  	              error = Read_Mode_Error_Logik(address,64);
