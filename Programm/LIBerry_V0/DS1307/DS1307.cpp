...
// PUBLIC FUNCTIONS
void DS1307::get(int *rtc, bool refresh)   // boolean  //Aquire data from buffer and convert to int, refresh buffer if required
{
  if(refresh) read();
...
int DS1307::get(int c, bool refresh) //boolean // aquire individual RTC item from buffer, return as int, refresh buffer if required
{
  if(refresh) read();
  int v=-1;
  switch(c)
